...
function getFeaturedFeelings() {
  return getFeelings()
    ...
    .map((feeling, index) => {
      ...
      return { feeling, index, count: realProducts.length };
    })
...
export function HomeFeelingCards() {
  ...
  return (
    ...
    >
      ...
      {feelings.map(({ feeling, count }, index) => {
        ...
        return (
          ...
          >
            ...
            <div className="relative mt-auto flex flex-col gap-1">
              <span className="font-label text-[10px] font-medium uppercase tracking-[0.2em] text-white/90 drop-shadow-sm md:text-[11px]">
                {count} {count === 1 ? 'design' : 'designs'}
              </span>
              <span className="font-headline text-[1.45rem] font-semibold leading-tight tracking-tight text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.45)] md:text-[2rem]">
                {feeling.name}
              </span>
            </div>
          </Link>
