...
export function PdpDeliveryPaymentCard({
  ...
  return (
    <div className="rounded-[8px] border border-[#ded7ce] bg-white/88 p-[25px_27px] shadow-[0_1px_2px_rgba(32,25,18,.04),0_12px_26px_rgba(32,25,18,.04)]">
      <h2 className="mb-[16px] font-headline text-[33px] font-normal leading-1 tracking-[-1.1px] text-obsidian">
        Delivery &amp; Payment
      </h2>

      {/* Delivery info row */}
      <div className="grid grid-cols-[52px_1fr] gap-[22px] items-start">
        <div className="flex h-[45px] w-[45px] items-center justify-center rounded-full bg-[#ece7de] text-[21px] text-[#534f49]" aria-hidden>
        ...
        <ul className="m-0 pl-[18px]">
          <li className="my-[1px] text-[14px] text-[#3f3a35]">
          ...
          {deliveryDynamic ? (
            <li className="my-[1px] text-[14px] text-[#3f3a35]">{deliveryDynamic.urgencyLine}</li>
          ) : null}
          {(trustItems as readonly string[]).some(t => t.includes('COD')) || PDP_SCHEMA.trustStripItems.some(t => t.includes('COD')) ? (
            <li className="my-[1px] text-[14px] text-[#3f3a35]">COD available</li>
          ) : null}
          <li className="my-[1px] text-[14px] text-[#3f3a35]">Card / wallet payment available</li>
        </ul>
      </div>

      {/* Divider */}
      <hr className="my-[18px_0_13px] border-0 border-t border-[#ded7ce]" />
      ...
      <div className="grid grid-cols-[52px_1fr] gap-[22px] items-start">
        ...
        <div>
          <h3 className="mb-[4px] font-headline text-[22px] font-normal tracking-[-1px] text-obsidian">Exchange</h3>
          <ul className="m-0 pl-[18px]">
            {copy.shippingSections.map((section) => (
              <li key={section.title} className="my-[1px] text-[14px] text-[#3f3a35]">
                {section.body}
              </li>
            ))}
            {hasWhatsapp ? (
              <li>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-[14px] text-deep-teal underline decoration-deep-teal/35 underline-offset-[3px] transition-colors hover:text-obsidian"
                >
      ...
      <p className="mt-4 text-[12px] text-[#5a554f]/60">{copy.deliveryEstimateNote}</p>
    </div>
  );
}
