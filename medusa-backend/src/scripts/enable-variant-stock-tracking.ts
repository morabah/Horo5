...
function parseDefaultQty(args: unknown): number {
  ...
  const positional = arr.find((a) => typeof a === "string" && a !== "dryrun" && !a.startsWith("-"));
...
export default async function enableVariantStockTracking({
  ...
  logger.info(
    `enable-variant-stock-tracking: defaultQty=${defaultQty}${dryRun ? " (DRY RUN)" : ""}`,
  );
  ...
  const inventoryItemIdToVariantSku = new Map<string, string>();
  for (const variant of refreshedVariants) {
    ...
    for (const link of variant.inventory_items ?? []) {
      ...
      if (variant.sku) {
        inventoryItemIdToVariantSku.set(itemId, variant.sku);
      }
    }
  }
