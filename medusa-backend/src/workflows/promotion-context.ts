...
export function buildPromotionContext(cart: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const line of items) {
    ...
    const meta =
      product && typeof product === "object" && !Array.isArray(product)
        ...
        : undefined
    if (meta && typeof meta.feelingSlug === "string" && meta.feelingSlug.trim()) {
      feelingSlugs.add(meta.feelingSlug.trim())
    }
    ...
    if (
      ...
      lineMeta.feelingSlug.trim()
    ) {
      feelingSlugs.add(lineMeta.feelingSlug.trim())
    }
  }
...
updateCartPromotionsWorkflow.hooks.setPromotionContext(
  ({ cart }, { container }) => {
    return new StepResponse(buildPromotionContext(cart))
  }
)
