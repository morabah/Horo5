...
export async function writeCatalogSyncReport(reportPath: string | undefined, report: CatalogSyncReport, cwd: string): Promise<void> {
...
export async function runCatalogSync({
  ...
  const canRunStages = !options.dryRun && (options.allowPartial || issues.length === 0)

  if (!options.dryRun && !options.allowPartial && issues.length > 0) {
    stageErrors.push("Medusa stages skipped: validation issues exist and allowPartial=false.")
  }

  if (canRunStages && importHandles.length) {
    if (!container) {
      stageErrors.push("Import skipped: Medusa container is not available.")
    } else {
      try {
        const { default: importProductDrops } = await import("../../scripts/import-product-drops.js") as unknown as {
  ...
  if (canRunStages && !options.noStock && Object.keys(stockMap).length) {
    if (!container) {
      stageErrors.push("Stock skipped: Medusa container is not available.")
    } else {
      try {
        const { runEnableVariantStockTracking } = await import("../../scripts/enable-variant-stock-tracking.js")
  ...
  if (canRunStages && !options.noInventory && Object.keys(stockMap).length) {
    if (!container) {
      stageErrors.push("Inventory skipped: Medusa container is not available.")
    } else {
      try {
        const { runBackfillInventoryLevels } = await import("../../scripts/backfill-inventory-levels.js")
